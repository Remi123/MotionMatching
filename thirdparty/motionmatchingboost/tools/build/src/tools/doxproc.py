...
def get_args(argv=sys.argv[1:]):
    spec = ["xmldir=", "output=", "id=", "title=", "enable-index", "help"]
    options = {"--xmldir": "xml", "--output": None, "--id": "dox", "--title": "Doxygen"}
...
"""
        ...
        if not self.generated:
            ...
            for symbol in symbols:
                if self.symbols[symbol]["kind"] not in ("namespace", "header"):
                    container = self._resolveContainer(
                        self.symbols[symbol], self.symbols[self.symbols[symbol]["header"]]["dom"]
                    )
    ...
    def _resolveContainer(self, cpp, root):
        ...
        for ns in cpp["namespace"]:
            ...
            if not node:
                node = container.appendChild(self._createNode("namespace", name=ns))
            container = node
        ...
        while i < len(entries):
            ...
            while i < len(entries) and dividerKey == entries[i]["name"][0].upper():
                ...
                while i < len(entries) and entries[i]["name"] == iename:
                    ...
                    ie.appendChild(self._createText("link", entries[i]["compoundname"], linkend=entries[i]["id"]))
                    ie.appendChild(self.boostbook.createTextNode(")"))
                    i += 1
    ...
    def _translate_compounddef_includes_(self, node, target=None, **kwargs):
        ...
        if name not in self.symbols:
            ...
            self.symbols[name] = {
                ...
                "dom": self._createNode("header", id=node.getAttribute("refid"), name=name),
            }
        return None
    ...
    def _translate_basecompoundref(self, ref, target=None, **kwargs):
        inherit = target.appendChild(self._createNode("inherit", access=ref.getAttribute("prot")))
        ...
        for param in templateparamlist.childNodes:
            if param.nodeName == "param":
                ...
                templateParam = template.appendChild(
                    self._createNode(paramKind, name=self._getChildData("declname", root=param))
                )
                if paramKind == "template-nontype-parameter":
                    template_type = templateParam.appendChild(self._createNode("type"))
                    self._translate_type(self._getChild("type", root=param), target=template_type)
                if defval:
    ...
    def _translate_sectiondef_public_func(self, sectiondef, target=None, **kwargs):
        return self._translate_sectiondef_func_(sectiondef, name="public member functions", target=target, **kwargs)
    ...
    def _translate_sectiondef_public_static_func(self, sectiondef, target=None, **kwargs):
        return self._translate_sectiondef_func_(sectiondef, name="public static functions", target=target, **kwargs)

    # ~ Translate:
    # ~   <sectiondef kind="protected-func">...</sectiondef>
    def _translate_sectiondef_protected_func(self, sectiondef, target=None, **kwargs):
        return self._translate_sectiondef_func_(sectiondef, name="protected member functions", target=target, **kwargs)
    ...
    def _translate_sectiondef_private_static_func(self, sectiondef, target=None, **kwargs):
        return self._translate_sectiondef_func_(sectiondef, name="private static functions", target=target, **kwargs)

    # ~ Translate:
    # ~   <sectiondef kind="public-func">...</sectiondef>
    def _translate_sectiondef_private_func(self, sectiondef, target=None, **kwargs):
        return self._translate_sectiondef_func_(sectiondef, name="private member functions", target=target, **kwargs)
    ...
    def _translate_sectiondef_user_defined(self, sectiondef, target=None, **kwargs):
        return self._translate_sectiondef_func_(
            sectiondef, name=self._getChildData("header", root=sectiondef), target=target, **kwargs
        )
    ...
    def _translate_memberdef_typedef(self, memberdef, target=None, scope=None, **kwargs):
        self._setID(memberdef.getAttribute("id"), scope + "::" + self._getChildData("name", root=memberdef))
        typedef = target.appendChild(
            self._createNode(
                "typedef", id=memberdef.getAttribute("id"), name=self._getChildData("name", root=memberdef)
            )
        )
    ...
    def _translate_memberdef_function(self, memberdef, target=None, scope=None, **kwargs):
        ...
        method = target.appendChild(
            self._createNode(
                ...
                name=name,
                cv=" ".join([if_attribute(memberdef, "const", "const", "").strip()]),
                specifiers=" ".join(
                    [
    ...
    def _translate_memberdef_function_templateparamlist(self, templateparamlist, target=None, **kwargs):
    ...
    def _translate_memberdef_variable(self, memberdef, target=None, scope=None, **kwargs):
        self._setID(memberdef.getAttribute("id"), scope + "::" + self._getChildData("name", root=memberdef))
        data_member = target.appendChild(
            self._createNode(
                "data-member", id=memberdef.getAttribute("id"), name=self._getChildData("name", root=memberdef)
            )
        )
    ...
    def _translate_memberdef_enum(self, memberdef, target=None, scope=None, **kwargs):
        self._setID(memberdef.getAttribute("id"), scope + "::" + self._getChildData("name", root=memberdef))
        enum = target.appendChild(
            self._createNode("enum", id=memberdef.getAttribute("id"), name=self._getChildData("name", root=memberdef))
        )
        for n in memberdef.childNodes:
    ...
    def _translate_memberdef_enum_enumvalue(self, enumvalue, target=None, scope=None, **kwargs):
        self._setID(enumvalue.getAttribute("id"), scope + "::" + self._getChildData("name", root=enumvalue))
        value = target.appendChild(
            self._createNode(
                "enumvalue", id=enumvalue.getAttribute("id"), name=self._getChildData("name", root=enumvalue)
            )
        )
        initializer = self._getChild("initializer", root=enumvalue)
        if initializer:
            self._translateChildren(initializer, target=target.appendChild(self._createNode("default")))
        return value
    ...
    def _translate_param(self, param, target=None, **kwargs):
        parameter = target.appendChild(self._createNode("parameter", name=self._getChildData("declname", root=param)))
    ...
    def _translate_type(self, type, target=None, **kwargs):
        ...
        if (
            xml.startswith("<type>boost::mpl::")
            or xml.startswith("<type>BOOST_PP_")
            or re.match("<type>boost::(lazy_)?(enable|disable)_if", xml)
        ):
    ...
    def _cppName(self, type):
        ...
        if result["namespace"] and len(result["namespace"]) > 0:
            ...
            while len(result["namespace"]) > 0 and (
                namespace not in self.symbols or self.symbols[namespace]["kind"] != "namespace"
            ):
...
def main(xmldir=None, output=None, id=None, title=None, index=False):
    ...
    namespace_files = filter(lambda x: os.path.basename(x).startswith("namespace"), input)
    decl_files = filter(
        lambda x: not os.path.basename(x).startswith("namespace") and not os.path.basename(x).startswith("_"), input
    )
