...
## glob wildcards:3
test_glob_archive(
    [archive1], "[ SORT [ GLOB_ARCHIVE $archive1 : b* ] ]", ["$archive1(b$obj)", "$archive1(b_match$obj)"]
)

## glob multiple patterns with multiple results.
test_glob_archive(
    [archive1], "[ SORT [ GLOB_ARCHIVE $archive1 : b.* b_* ] ]", ["$archive1(b$obj)", "$archive1(b_match$obj)"]
)
...
test_glob_archive(
    ...
    ["$archive1(b$obj)", "$archive1(b_match$obj)", "$archive2(b$obj)", "$archive2(b_match$obj)"],
)
...
test_glob_archive(
    [archive1],
    "[ SORT [ GLOB_ARCHIVE $archive1 : seq_check*$obj ] ]",
    ["$archive1(seq_check1$obj)", "$archive1(seq_check2$obj)", "$archive1(seq_check3$obj)"],
)
...
if symbol_glob_supported:
    ...
    test_glob_archive(
        [archive1], "[ GLOB_ARCHIVE $archive1 : *symbol* : : *member* ]", ["$archive1(members_and_symbols_check$obj)"]
    )

    ## case insensitive symbol glob.
    test_glob_archive(
        [archive1], "[ GLOB_ARCHIVE $archive1 : : true : symbolcasecheck ]", ["$archive1(symbol_case_check$obj)"]
    )
