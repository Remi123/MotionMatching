...
namespace motionmatchingboost_ns
{
   template <class T>
   T* get_pointer(T* p)
   { return p; }
...
namespace user_ns
{
   ...
   int f(T x)
   {
      ...
      //using namespace motionmatchingboost;
      // this statement breaks ADL:
      using motionmatchingboost_ns::get_pointer;    // conforming compilers require
...
namespace motionmatchingboost_function_scope_using_declaration_breaks_adl{
...
int test()
{
   ...
   typedef void* pv;
   i = user_ns::f(pv());
   i = user_ns::f(motionmatchingboost_ns::inner2::X<int>());
   (void)i;
   return 0;
}
