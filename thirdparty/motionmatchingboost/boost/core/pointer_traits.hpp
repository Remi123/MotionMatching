...
struct ptr_to<T*, T, true> {
    static T* pointer_to(T& v) BOOST_NOEXCEPT {
        return motionmatchingboost::addressof(v);
    }
};
...
-> decltype(motionmatchingboost::pointer_traits<T>::to_address(v))
{
    return motionmatchingboost::pointer_traits<T>::to_address(v);
}
...
{
    return motionmatchingboost::detail::ptr_address(v.operator->(), 0);
}
...
{
    return motionmatchingboost::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return motionmatchingboost::to_address(v.operator->());
}
#endif
