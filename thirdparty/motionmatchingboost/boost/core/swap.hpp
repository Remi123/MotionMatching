...
// - motionmatchingboost::swap has two template arguments, instead of one, to
// avoid ambiguity when swapping objects of a Boost type that does
// not have its own motionmatchingboost::swap overload.
...
namespace motionmatchingboost_swap_impl
{
  ...
  void swap_impl(T (& left)[N], T (& right)[N])
    BOOST_CORE_SWAP_NOEXCEPT_IF(BOOST_NOEXCEPT_EXPR(::motionmatchingboost_swap_impl::swap_impl(left[0], right[0])))
  {
    for (std::size_t i = 0; i < N; ++i)
    {
      ::motionmatchingboost_swap_impl::swap_impl(left[i], right[i]);
    }
  }
}
...
namespace motionmatchingboost
{
  ...
  BOOST_GPU_ENABLED
  typename enable_if_c< !motionmatchingboost_swap_impl::is_const<T1>::value && !motionmatchingboost_swap_impl::is_const<T2>::value >::type
  swap(T1& left, T2& right)
    BOOST_CORE_SWAP_NOEXCEPT_IF(BOOST_NOEXCEPT_EXPR(::motionmatchingboost_swap_impl::swap_impl(left, right)))
  {
    ::motionmatchingboost_swap_impl::swap_impl(left, right);
  }
}
