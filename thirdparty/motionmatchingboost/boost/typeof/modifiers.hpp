...
#define BOOST_TYPEOF_modifier_support(ID, Fun)\
    ...
    {\
        typedef\
            typename motionmatchingboost::type_of::encode_type<\
            typename motionmatchingboost::type_of::push_back<\
            V\
            , motionmatchingboost::type_of::constant<std::size_t,ID> >::type\
            , T>::type\
            type;\
    };\
    template<class Iter> struct decode_type_impl<motionmatchingboost::type_of::constant<std::size_t,ID>, Iter>\
    {\
        typedef motionmatchingboost::type_of::decode_type<Iter> d1;\
...
#define BOOST_TYPEOF_array_support(ID, Qualifier)\
    ...
    {\
        typedef\
            ...
            V\
            , motionmatchingboost::type_of::constant<std::size_t,ID> >::type\
            , motionmatchingboost::type_of::constant<std::size_t,N> >::type\
            , T>::type\
        type;\
    };\
    template<class Iter>\
    struct decode_type_impl<motionmatchingboost::type_of::constant<std::size_t,ID>, Iter>\
    {\
        enum{n = Iter::type::value};\
        typedef motionmatchingboost::type_of::decode_type<typename Iter::next> d;\
