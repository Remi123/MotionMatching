...
//    const std::list<T>::iterator prev = motionmatchingboost::prior(p);
//    const std::list<T>::iterator next = motionmatchingboost::next(prev, 2);
...
struct next_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        motionmatchingboost::iterators::advance(x, n);
        return x;
    }
};
...
struct prior_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        ...
        motionmatchingboost::iterators::reverse_iterator< T > rx(x);
        motionmatchingboost::iterators::advance(rx, n);
        return rx.base();
    }
};
