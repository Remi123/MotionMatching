...
private:
    struct nil_t {};
    template<typename U> static auto  test( U* ) -> decltype(motionmatchingboost::declval<U>().size());
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::begin(motionmatchingboost::declval<U&>())); // does not work with motionmatchingboost::begin
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::end(motionmatchingboost::declval<U&>())); // does not work with motionmatchingboost::end
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
#else
    typedef decltype(motionmatchingboost::declval<
        typename motionmatchingboost::add_const<
...
struct bt_iterator_traits< T [N], true > {
    typedef typename motionmatchingboost::add_const<T>::type T_const;
    typedef typename motionmatchingboost::add_pointer<T_const>::type const_iterator;
