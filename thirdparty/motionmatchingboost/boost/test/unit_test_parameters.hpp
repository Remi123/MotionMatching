...
public:
    ...
    void            setup( const const_string& stream_name,
                           motionmatchingboost::function<void ()> const &cleaner_callback = motionmatchingboost::function<void ()>() )
    {
        ...
        if( stream_name == "stderr" ) {
            m_stream = &std::cerr;
            if(cleaner_callback) {
                m_cleaner = motionmatchingboost::make_shared<callback_cleaner>(cleaner_callback);
            }
            else {
                m_cleaner.reset();
            }
...
private:
    struct callback_cleaner {
        callback_cleaner(motionmatchingboost::function<void ()> cleaner_callback)
        ...
        motionmatchingboost::function<void ()> m_cleaner_callback;
        std::ofstream m_file;
    };

    // Data members
    motionmatchingboost::shared_ptr<callback_cleaner>   m_cleaner;
