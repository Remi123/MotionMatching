...
#   define DEDUCE_RESULT_TYPE( oper )                                   \
    decltype(motionmatchingboost::declval<Lhs>() oper motionmatchingboost::declval<Rhs>() ) optype; \
    typedef typename motionmatchingboost::remove_reference<optype>::type              \
/**/
#else
...
#define ADD_OP_SUPPORT( oper, name, _, _i )                     \
    template<typename T>                                        \
    binary_expr<ExprType,typename motionmatchingboost::decay<T const>::type,  \
        op::name<ValType,typename motionmatchingboost::decay<T const>::type> >\
    operator oper( T const& rhs ) const                         \
    {                                                           \
        typedef typename motionmatchingboost::decay<T const>::type Rhs;       \
