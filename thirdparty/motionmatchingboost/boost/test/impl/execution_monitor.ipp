...
extract( motionmatchingboost::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = motionmatchingboost::get_error_info<ErrorInfo>( *ex );
...
report_error( execution_exception::error_code ec, motionmatchingboost::exception const* be, char const* format, va_list* args )
{
...
report_error( execution_exception::error_code ec, motionmatchingboost::exception const* be, char const* format, ... )
{
    va_list args;
...
{
    return motionmatchingboost::core::demangle(typeid(t).name());
}
#endif
...
// ************************************************************************** //
// **************    motionmatchingboost::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************         motionmatchingboost::detail::signal_action         ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************        motionmatchingboost::detail::signal_handler         ************** //
// ************************************************************************** //
...
int
execution_monitor::catch_signals( motionmatchingboost::function<int ()> const& F )
{
...
int
execution_monitor::execute( motionmatchingboost::function<int ()> const& F )
{
    ...
    BOOST_TEST_I_TRY {
        ...
        motionmatchingboost::ignore_unused( G );
    ...
    // motionmatchingboost::exception (before std::exception, with extended diagnostic)
    catch( motionmatchingboost::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
                              "%s", motionmatchingboost::diagnostic_information(ex).c_str() ); }
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<motionmatchingboost::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
struct forward {
    explicit    forward( motionmatchingboost::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    motionmatchingboost::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( motionmatchingboost::function<void ()> const& F )
{
...
{
    motionmatchingboost::ignore_unused(mask);
