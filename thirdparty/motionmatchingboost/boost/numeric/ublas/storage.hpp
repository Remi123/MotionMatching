...
#pragma warning(pop)
                  ...
                  for (pointer d = data_; d != data_ + size_; ++d)
                      motionmatchingboost::allocator_construct(alloc_, d);
              }
          }
          else
              data_ = 0;
                    ...
                    if (preserve) {
                        ...
                        if (size < size_) {
                            for (; di != data_ + size; ++di) {
                                motionmatchingboost::allocator_construct(alloc_, di, *si);
                                ++si;
                            }
                        }
                        else {
                            ...
                            for (; di != data_ + size; ++di) {
                                motionmatchingboost::allocator_construct(alloc_, di, init);
                            }
                        }
                    }
                    else {
...
#pragma warning(pop)
                            ...
                            for (pointer di = data_; di != data_ + size; ++di)
                                motionmatchingboost::allocator_construct(alloc_, di);
                        }
                    }
                }
...
#pragma warning(pop)
                        ...
                        for (pointer si = p_data; si != p_data + size_; ++si)
                            motionmatchingboost::allocator_destroy(alloc_, si);
                    }
    ...
    public:
        ...
        size_type max_size () const {
            return motionmatchingboost::allocator_max_size(alloc_);
        }
    ...
    private:
        friend class motionmatchingboost::serialization::access;
...
// MSVC does not like arrays of size 0 in base classes.  Hence, this conditionally changes the size to 1
    ...
    class index_pair_array:
        private motionmatchingboost::noncopyable {
