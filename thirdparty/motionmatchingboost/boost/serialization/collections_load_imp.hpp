...
collection_load_impl(
    ...
    hint = t.begin();
    while(count-- > 0){
        ar >> motionmatchingboost::serialization::make_nvp("item", *hint++);
    }
}
...
collection_load_impl(
    ...
    while(count-- > 0){
        ...
        ar >> motionmatchingboost::serialization::make_nvp("item", u.reference());
        t.push_back(motionmatchingboost::move(u.reference()));
