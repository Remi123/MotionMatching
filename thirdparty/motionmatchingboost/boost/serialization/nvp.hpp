...
#define BOOST_SERIALIZATION_NVP(name)                       \
    motionmatchingboost::serialization::make_nvp(BOOST_PP_STRINGIZE(name), name)
/**/

#define BOOST_SERIALIZATION_BASE_OBJECT_NVP(name)           \
    motionmatchingboost::serialization::make_nvp(                         \
        BOOST_PP_STRINGIZE(name),                           \
        motionmatchingboost::serialization::base_object<name >(*this)     \
    )
/**/
...
struct is_wrapper<const nvp<T> > {
    typedef motionmatchingboost::mpl::true_ type;
};
template<class T>
