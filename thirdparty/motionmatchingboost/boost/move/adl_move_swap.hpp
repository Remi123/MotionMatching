...
#include <boost/move/utility_core.hpp> //for motionmatchingboost::move

#if !defined(BOOST_MOVE_DOXYGEN_INVOKED)

#if defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
namespace motionmatchingboost_move_member_swap {
...
namespace motionmatchingboost_move_adl_swap{
...
template<class T>
BOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y, typename motionmatchingboost::move_detail::enable_if_c<!motionmatchingboost::move_detail::has_move_emulation_enabled_impl<T>::value>::type* = 0)
{
...
BOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename motionmatchingboost::move_detail::enable_if< and_op_not_impl<motionmatchingboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                        , motionmatchingboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  T t(::motionmatchingboost::move(x)); x = ::motionmatchingboost::move(y); y = ::motionmatchingboost::move(t);  }

template<class T>
BOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename motionmatchingboost::move_detail::enable_if< and_op_impl< motionmatchingboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                    , motionmatchingboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  x.swap(y);  }
...
void swap_proxy(T (& x)[N], T (& y)[N])
{
   for (std::size_t i = 0; i < N; ++i){
      ::motionmatchingboost_move_adl_swap::swap_proxy(x[i], y[i]);
   }
}
...
//!      <code>T t(::motionmatchingboost::move(x)); x = ::motionmatchingboost::move(y); y = ::motionmatchingboost::move(t);</code>.
template<class T>
BOOST_MOVE_FORCEINLINE void adl_move_swap(T& x, T& y)
{
   ::motionmatchingboost_move_adl_swap::swap_proxy(x, y);
}

//! Exchanges elements between range [first1, last1) and another range starting at first2
//! using motionmatchingboost::adl_move_swap.
//! 
//! Parameters:
...
{
    while (first1 != last1) {
      ::motionmatchingboost::adl_move_swap(*first1, *first2);
      ++first1;
      ++first2;
    }
   return first2;
...
{
   while (first1 != last1) {
      ::motionmatchingboost::adl_move_swap(*(--last1), *(--last2));
   }
   return last2;
}
...
{
   motionmatchingboost::adl_move_swap(*a, *b); 
}
