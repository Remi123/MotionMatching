...
//
namespace motionmatchingboost{ namespace type_traits_detail{
...
{
  template<typename Q> static motionmatchingboost::true_type  test(decltype(&Q::operator())*);
  template<typename>   static motionmatchingboost::false_type test(...);
...
struct is_likely_stateless_lambda<
  T,
  typename motionmatchingboost::enable_if_<has_one_operator_call<T>::value>::type> :
     motionmatchingboost::is_convertible<T, typename equivalent_function_pointer<T>::type
>{};
...
namespace motionmatchingboost {
   namespace type_traits_detail {
      ...
      template<typename T>
      struct is_likely_stateless_lambda : public motionmatchingboost::integral_constant<bool, false> {};
}}
