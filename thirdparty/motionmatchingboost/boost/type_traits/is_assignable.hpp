...
namespace motionmatchingboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename U, typename = decltype(motionmatchingboost::declval<T>() = motionmatchingboost::declval<U>())>
   ...
   template <class T, class U> struct is_assignable : public integral_constant<bool, sizeof(detail::is_assignable_imp::test<T, U>(0)) == sizeof(motionmatchingboost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(motionmatchingboost::is_complete<T>::value, "Arguments to is_assignable must be complete types");
   };
