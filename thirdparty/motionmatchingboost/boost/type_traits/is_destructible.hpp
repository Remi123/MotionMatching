...
namespace motionmatchingboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename = decltype(motionmatchingboost::declval<T&>().~T())>
   ...
   template <class T> struct is_destructible : public integral_constant<bool, sizeof(motionmatchingboost::detail::is_destructible_imp::test<T>(0)) == sizeof(motionmatchingboost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(motionmatchingboost::is_complete<T>::value, "Arguments to is_destructible must be complete types");
   };
