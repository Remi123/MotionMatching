...
struct const_traits {
    ...
    typedef typename motionmatchingboost::allocator_const_pointer<Alloc>::type pointer;
    typedef const value_type& reference;
    typedef typename motionmatchingboost::allocator_size_type<Alloc>::type size_type;
    typedef typename motionmatchingboost::allocator_difference_type<Alloc>::type difference_type;
...
struct nonconst_traits {
    // Basic types
    typedef typename Alloc::value_type value_type;
    typedef typename motionmatchingboost::allocator_pointer<Alloc>::type pointer;
...
struct assign_range {
    ...
    template <class Pointer>
    void operator () (Pointer p) const {
        motionmatchingboost::cb_details::uninitialized_copy(m_first, m_last, p, m_alloc);
    }
};
...
inline ForwardIterator uninitialized_copy(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    ...
    BOOST_TRY {
        for (; first != last; ++first, ++dest)
            motionmatchingboost::allocator_construct(a, motionmatchingboost::to_address(dest), *first);
    } BOOST_CATCH(...) {
        for (; next != dest; ++next)
            motionmatchingboost::allocator_destroy(a, motionmatchingboost::to_address(next));
        BOOST_RETHROW
    }
    BOOST_CATCH_END
    return dest;
...
ForwardIterator uninitialized_move_if_noexcept_impl(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a,
    true_type) {
    for (; first != last; ++first, ++dest)
        motionmatchingboost::allocator_construct(a, motionmatchingboost::to_address(dest), motionmatchingboost::move(*first));
    return dest;
}
...
template<class InputIterator, class ForwardIterator, class Alloc>
ForwardIterator uninitialized_move_if_noexcept(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    typedef typename motionmatchingboost::is_nothrow_move_constructible<typename Alloc::value_type>::type tag_t;
...
inline void uninitialized_fill_n_with_alloc(ForwardIterator first, Diff n, const T& item, Alloc& alloc) {
    ...
    BOOST_TRY {
        for (; n > 0; ++first, --n)
            motionmatchingboost::allocator_construct(alloc, motionmatchingboost::to_address(first), item);
    } BOOST_CATCH(...) {
        for (; next != first; ++next)
            motionmatchingboost::allocator_destroy(alloc, motionmatchingboost::to_address(next));
        BOOST_RETHROW
    }
    BOOST_CATCH_END
}
