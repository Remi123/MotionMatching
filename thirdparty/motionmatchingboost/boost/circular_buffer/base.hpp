...
private empty_value<Alloc>
{
  ...
  // Requirements
    //BOOST_CLASS_REQUIRE(T, motionmatchingboost, SGIAssignableConcept);
            ...
            for (pointer first = m_first; dest < src; src = first) {
                for (size_type ii = 0; src < m_end; ++src, ++dest, ++moved, ++ii) {
                    ...
                    if (is_uninitialized(dest)) {
                        motionmatchingboost::allocator_construct(alloc(), motionmatchingboost::to_address(dest), motionmatchingboost::move_if_noexcept(*src));
                        ++constructed;
                    } else {
                        value_type tmp = motionmatchingboost::move_if_noexcept(*src); 
                        replace(src, motionmatchingboost::move_if_noexcept(*dest));
                        replace(dest, motionmatchingboost::move(tmp));
                    }
                }
            }
        } BOOST_CATCH(...) {
    ...
    */
    void rotate(const_iterator new_begin) {
        ...
        if (full()) {
            ...
            if (m < n) {
                for (; m > 0; --m) {
                    push_front(motionmatchingboost::move_if_noexcept(back()));
                    pop_back();
                }
            } else {
                for (; n > 0; --n) {
                    push_back(motionmatchingboost::move_if_noexcept(front()));
                    pop_front();
                }
            }
        }
...
// Size and capacity
    ...
    */
    size_type max_size() const BOOST_NOEXCEPT {
        return (std::min<size_type>)(motionmatchingboost::allocator_max_size(alloc()), (std::numeric_limits<difference_type>::max)());
    }
...
// Construction/Destruction
    ...
    */
    ...
    : base(motionmatchingboost::empty_init_t(), alloc), m_buff(0), m_end(0), m_first(0), m_last(0), m_size(0) {}
    ...
    */
    ...
    : base(motionmatchingboost::empty_init_t(), alloc), m_size(0) {
    ...
    */
    ...
    : base(motionmatchingboost::empty_init_t(), alloc), m_size(n) {
...
#endif
    base(motionmatchingboost::empty_init_t(), cb.get_allocator()),
    m_size(cb.size()) {
    ...
    */
    ...
    : base(motionmatchingboost::empty_init_t(), cb.get_allocator()), m_buff(0), m_end(0), m_first(0), m_last(0), m_size(0) {
        cb.swap(*this);
    }
    ...
    */
    ...
    : base(motionmatchingboost::empty_init_t(), alloc) {
...
private:
    ...
    void push_back_impl(ValT item) {
        if (full()) {
            ...
            motionmatchingboost::allocator_construct(alloc(), motionmatchingboost::to_address(m_last), static_cast<ValT>(item));
            increment(m_last);
            ++m_size;
        }        
    }
    ...
    void push_front_impl(ValT item) {
        BOOST_TRY {
            if (full()) {
                ...
                decrement(m_first);
                motionmatchingboost::allocator_construct(alloc(), motionmatchingboost::to_address(m_first), static_cast<ValT>(item));
                ++m_size;
            }
        } BOOST_CATCH(...) {
            increment(m_first);
...
public:
    ...
    */
    void push_back(rvalue_type item) {
        push_back_impl<rvalue_type>(motionmatchingboost::move(item));
    }
    ...
    */
    void push_back() {
        value_type temp;
        push_back(motionmatchingboost::move(temp));
    }
    ...
    */
    void push_front(rvalue_type item) {
        push_front_impl<rvalue_type>(motionmatchingboost::move(item));
    }
    ...
    */
    void push_front() {
        value_type temp;
        push_front(motionmatchingboost::move(temp));
    }
...
// Insert
    ...
    */
    iterator insert(iterator pos, rvalue_type item) {
        return insert_impl<rvalue_type>(pos, motionmatchingboost::move(item));
    }
    ...
    */
    iterator insert(iterator pos) {
        value_type temp;
        return insert(pos, motionmatchingboost::move(temp));
    }
...
private:
    ...
    iterator rinsert_impl(iterator pos, ValT item) {
        ...
        if (pos == begin()) {
            ...
            BOOST_TRY {
                while (src != pos.m_it) {
                    construct_or_replace(construct, dest, motionmatchingboost::move_if_noexcept(*src));
                    increment(src);
                    increment(dest);
                    construct = false;
                }
...
public:
    ...
    */
    iterator rinsert(iterator pos, rvalue_type item) {
        return rinsert_impl<rvalue_type>(pos, motionmatchingboost::move(item));
    }
    ...
    */
    iterator rinsert(iterator pos) {
        value_type temp;
        return rinsert(pos, motionmatchingboost::move(temp));
    }
...
// Erase
    ...
    */
    iterator erase(iterator pos) {
        ...
        for (pointer p = pos.m_it; next != m_last; p = next, increment(next))
            replace(p, motionmatchingboost::move_if_noexcept(*next));
        decrement(m_last);
        destroy_item(m_last);
        --m_size;
#if BOOST_CB_ENABLE_DEBUG
    ...
    */
    iterator erase(iterator first, iterator last) {
        ...
        while (last.m_it != 0)
            replace((first++).m_it, motionmatchingboost::move_if_noexcept(*last++));
        do {
            decrement(m_last);
            destroy_item(m_last);
            --m_size;
    ...
    */
    iterator rerase(iterator pos) {
        ...
        for (decrement(prev); p != m_first; p = prev, decrement(prev))
            replace(p, motionmatchingboost::move_if_noexcept(*prev));
        destroy_item(m_first);
        increment(m_first);
        --m_size;
#if BOOST_CB_ENABLE_DEBUG
    ...
    */
    iterator rerase(iterator first, iterator last) {
        ...
        while (first.m_it != m_first) {
            ...
            decrement(p);
            replace(p, motionmatchingboost::move_if_noexcept(*first.m_it));
        }
        do {
            destroy_item(m_first);
            increment(m_first);
...
// Helper methods
    ...
    void replace(pointer pos, rvalue_type item) {
        *pos = motionmatchingboost::move(item);
#if BOOST_CB_ENABLE_DEBUG
        invalidate_iterators(iterator(this, pos));
#endif
    }
    ...
    void construct_or_replace(bool construct, pointer pos, param_value_type item) {
        if (construct)
            motionmatchingboost::allocator_construct(alloc(), motionmatchingboost::to_address(pos), item);
        else
            replace(pos, item);
    }
    ...
    void construct_or_replace(bool construct, pointer pos, rvalue_type item) {
        if (construct)
            motionmatchingboost::allocator_construct(alloc(), motionmatchingboost::to_address(pos), motionmatchingboost::move(item));
        else
            replace(pos, motionmatchingboost::move(item));
    }

    /*! INTERNAL ONLY */
    void destroy_item(pointer p) {
        motionmatchingboost::allocator_destroy(alloc(), motionmatchingboost::to_address(p));
#if BOOST_CB_ENABLE_DEBUG
    ...
    void initialize(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        initialize(distance, motionmatchingboost::make_move_iterator(tmp.begin()), motionmatchingboost::make_move_iterator(tmp.end()), distance);
    }
    ...
    void initialize(capacity_type buffer_capacity,
        ...
        while (first != last && !full()) {
            motionmatchingboost::allocator_construct(alloc(), motionmatchingboost::to_address(m_last), *first++);
            increment(m_last);
            ++m_size;
        }
        while (first != last) {
    ...
    void assign(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        assign_n(distance, distance,
            cb_details::make_assign_range
                (motionmatchingboost::make_move_iterator(tmp.begin()), motionmatchingboost::make_move_iterator(tmp.end()), alloc()));
    }
        ...
        if (pos.m_it == 0) {
            ...
            BOOST_TRY {
                for (; ii < construct; ++ii, increment(p))
                    motionmatchingboost::allocator_construct(alloc(), motionmatchingboost::to_address(p), *wrapper());
