...
struct sp_array_element {
    typedef typename motionmatchingboost::remove_cv<typename
        motionmatchingboost::remove_extent<T>::type>::type type;
};
...
struct sp_array_alignment {
    enum {
        value = sp_max_size<motionmatchingboost::alignment_of<T>::value,
            motionmatchingboost::alignment_of<U>::value>::value
    };
};
...
class sp_array_creator {
    ...
    typedef typename motionmatchingboost::type_with_alignment<sp_array_alignment<T,
...
private:
    typename motionmatchingboost::allocator_rebind<A, type>::type other_;
    std::size_t size_;
};
...
public:
    ...
    sp_array_base(const A& other, type* start, std::size_t size)
        : state_(other, size) {
        motionmatchingboost::alloc_construct_n(state_.allocator(),
            motionmatchingboost::first_scalar(start),
    ...
    sp_array_base(const A& other, type* start, std::size_t size, const U& list)
        ...
        motionmatchingboost::alloc_construct_n(state_.allocator(),
            motionmatchingboost::first_scalar(start), state_.size() * count,
            motionmatchingboost::first_scalar(&list), count);
    }
    ...
    void dispose() BOOST_SP_NOEXCEPT BOOST_OVERRIDE {
        motionmatchingboost::alloc_destroy_n(state_.allocator(),
            motionmatchingboost::first_scalar(sp_array_start<type>(this)),
            state_.size() * sp_array_count<type>::value);
    }
...
{
    return motionmatchingboost::allocate_shared<T>(motionmatchingboost::noinit_adapt(allocator), count);
}
...
{
    return motionmatchingboost::allocate_shared<T>(motionmatchingboost::noinit_adapt(allocator));
}
