...
public:
    ...
    explicit sp_reference_wrapper( T & t): t_( motionmatchingboost::addressof( t ) )
    {
    }
...
{
    motionmatchingboost::uintptr_t v = reinterpret_cast<motionmatchingboost::uintptr_t>( p );

    // match motionmatchingboost::hash<T*>
...
public:
...
#endif
    {
        ...
        {
            motionmatchingboost::checked_delete( p );
            throw;
        }
        ...
        {
            motionmatchingboost::checked_delete( p );
            motionmatchingboost::throw_exception( std::bad_alloc() );
        }
    ...
    template<class Y, class D>
    explicit shared_count( motionmatchingboost::movelib::unique_ptr<Y, D> & r ): pi_( 0 )
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        , id_(shared_count_id)
#endif
    {
        ...
        D2 d2( r.get_deleter() );
        pi_ = new sp_counted_impl_pd< typename motionmatchingboost::movelib::unique_ptr<Y, D>::pointer, D2 >( r.get(), d2 );
...
#endif
{
    ...
    {
        motionmatchingboost::throw_exception( motionmatchingboost::bad_weak_ptr() );
    }
}
