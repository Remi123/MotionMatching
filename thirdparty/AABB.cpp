...
namespace aabb
{
    ...
    std::vector<float> AABB::computeCentre()
    {
        std::vector<float> position(lowerBound.size());
    ...
    Tree::Tree(unsigned int dimension_,
               ...
               const std::vector<float>& boxSize_,
               unsigned int nParticles,
               bool touchIsOverlap_) :
    ...
    void Tree::setBoxSize(const std::vector<float>& boxSize_)
    {
        boxSize = boxSize_;
    }
    ...
    void Tree::insertParticle(unsigned int particle, std::vector<float>& position, float radius)
    {
    ...
    void Tree::insertParticle(unsigned int particle, std::vector<float>& lowerBound, std::vector<float>& upperBound)
    {
        // Make sure the particle doesn't already exist.
        if (particleMap.count(particle) != 0)
        {
    ...
    bool Tree::updateParticle(unsigned int particle, std::vector<float>& position, float radius,
                              bool alwaysReinsert)
    {
        // Validate the dimensionality of the position vector.
        if (position.size() != dimension)
        ...
        // AABB bounds vectors.
        std::vector<float> lowerBound(dimension);
        std::vector<float> upperBound(dimension);
    ...
    bool Tree::updateParticle(unsigned int particle, std::vector<float>& lowerBound,
                              std::vector<float>& upperBound, bool alwaysReinsert)
    {
        // Validate the dimensionality of the bounds vectors.
        if ((lowerBound.size() != dimension) && (upperBound.size() != dimension))
        {
    ...
    void Tree::periodicBoundaries(std::vector<float>& position)
    {
        for (unsigned int i=0;i<dimension;i++)
        {
            if (position[i] < 0)
    ...
    bool Tree::minimumImage(std::vector<float>& separation, std::vector<float>& shift)
    {
