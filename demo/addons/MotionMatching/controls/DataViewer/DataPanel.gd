...
func show_data(lib:MMAnimationLibrary,animname:String,time:float):
	tree.clear()
	tree.columns = 1
	#
	...
	var data := PackedFloat32Array()  #lib.MotionData.slice(lign*lib.nb_dimensions,lign*lib.nb_dimensions+lib.nb_dimensions)
	var anim :Animation= lib.get_animation(animname)

	
	for f in lib.motion_features as Array[MotionFeature]:
		var feature_data := PackedFloat32Array()
		feature_data.resize(f.get_dimension())
		data.append_array(f.bake_pose(lib,animname,time))
	...
	category_item.set_text(0, str(0))
	...
	timestamp_item.set_text(0, str(0))
	
	var biases_folder := tree.create_item(root,3)
	biases_folder.set_text(0,"Biases")
	biases_folder.set_custom_bg_color(0,Color.WHITE,true)
	var biases_item :TreeItem= tree.create_item(biases_folder)
	#biases_item.set_text(0, str(lib.db_biases[lign]))
	biases_item.set_text(0, str(0))
	...
	for f in lib.motion_features as Array[MotionFeature]:
		...
		for i in range(f.get_dimension()):
			var scale :float = lib.dimension_stddev[offset+i]
			var avg :float = lib.dimension_means[offset+i]
