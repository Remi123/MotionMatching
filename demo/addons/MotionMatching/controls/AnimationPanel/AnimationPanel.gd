...
signal on_animation_selected(lib: MMAnimationLibrary,name:String)
...
func _ready() -> void:
	...
	tree.item_activated.connect(_on_row_selected)
	pass

func _on_row_selected():
	var treeitem :TreeItem= tree.get_selected()
	var index := treeitem.get_index()
	prints(al.get_animation_list()[index],treeitem.get_text(Column.Name))
	on_animation_selected.emit(al,al.get_animation_list()[index])
